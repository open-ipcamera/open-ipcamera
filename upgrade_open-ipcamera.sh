...
if [[ "\$(echo \$VERSIONINSTALLED|tr -d '.')" -le "\$(echo \$VERSIONREPO|tr -d '.')" ]]; then
	...
	ssh pi@\$CAMERAIPV4 "cd \$PATHINSTALLDIR;cp -p ./variables.sh ./variables.sh.version-\$VERSIONREPO"
	...
	VARIABLESMODIFIED=\$(ssh pi@\$CAMERAIPV4 "cd \$PATHINSTALLDIR;diff --new-line-format="" --unchanged-line-format="" ./variables.sh.PRODUCTION ./variables.sh.v\$VERSIONREPO| grep "^[^#].*" ")
	
	# If files differ perform a 2-way merge of lines not shared:
	if [ $(echo \$VARIABLESMODIFIED) = '' ]; then ssh pi@\$CAMERAIPV4 "cd \$PATHINSTALLDIR;paste -d'\n' ./variables.sh.PRODUCTION ./variables.sh.v\$VERSIONREPO|awk -F'=' '!seen[\$1]++' > ./variables.sh"; fi
